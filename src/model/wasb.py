...
def build_wasb(args):
    config_path = '/home/s224705071/github/PhysicsInformedDeformableAttentionNetwork/src/config/wasb.yaml'
    with open(config_path, 'r') as f:
        cfg = yaml.safe_load(f)
        cfg['frames_in'] = args.num_frames
        cfg['frames_out'] = args.num_frames
    cfg = AttrDict(cfg)
    model = HRNet(cfg).to(args.device)
